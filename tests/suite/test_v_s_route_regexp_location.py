...
class TestRegexpLocation:
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                {
                    "regex_type": "exact",
                    "expected_results": {
                        "/exact-match$request": 200,
                        "/exact-match$request/": 404,
                        "/exact-match$request?var1=value": 200,
                    },
                },
                id="exact",
            ),
            pytest.param(
                {
                    "regex_type": "case-sensitive",
                    "expected_results": {
                        "/case-SENsitiVe/match": 200,
    ...
    def test_response_for_regex_location(
        self,
        kube_apis,
        ...
        patch_virtual_server_from_yaml(
            kube_apis.custom_objects, v_s_route_setup.vs_name, vs_src_yaml, v_s_route_setup.namespace
        )
        patch_v_s_route_from_yaml(
            kube_apis.custom_objects, v_s_route_setup.route_s.name, vsr_src_yaml, v_s_route_setup.route_s.namespace
        )
        wait_before_test(1)
    ...
    def test_flow_for_invalid_vsr(
        ...
        vsr_event_text = (
            f"VirtualServerRoute {text_vsr_s} was rejected with error: "
            f'spec.subroutes[1].path: Duplicate value: "=/backends/exact-match$request"'
        )
        vs_src_yaml = f"{TEST_DATA}" f"/virtual-server-route-regexp-location/standard/virtual-server-exact.yaml"
        patch_virtual_server_from_yaml(
            kube_apis.custom_objects, v_s_route_setup.vs_name, vs_src_yaml, v_s_route_setup.namespace
        )
        vsr_src_yaml = (
        ...
        patch_v_s_route_from_yaml(
            kube_apis.custom_objects, v_s_route_setup.route_m.name, vsr_src_yaml, v_s_route_setup.route_m.namespace
        )
        wait_before_test(2)
...
class TestVSRRegexpMultipleMatches:
    def test_exact_match_overrides_all(
        self,
        kube_apis,
        ingress_controller_prerequisites,
    ...
    def test_regexp_overrides_prefix(
        self,
        kube_apis,
        ingress_controller_prerequisites,
        ingress_controller_endpoint,
...
class TestVSRSelectorRegexpLocation:
    @pytest.mark.parametrize(
        "test_data",
        [
            pytest.param(
    ...
    def test_response_for_regex_location(
        ...
        vs_src_yaml = (
            f"{TEST_DATA}"
            f"/virtual-server-route-selector/regexp-location/standard/virtual-server-{test_data['regex_type']}.yaml"
        )
        vsr_src_yaml = (
            f"{TEST_DATA}/virtual-server-route-selector/regexp-location/route-single-{test_data['regex_type']}.yaml"
        )
        patch_virtual_server_from_yaml(
            kube_apis.custom_objects, v_s_route_selector_setup.vs_name, vs_src_yaml, v_s_route_selector_setup.namespace
        )
        patch_v_s_route_from_yaml(
            kube_apis.custom_objects,
    ...
    def test_flow_for_invalid_vs(
        ...
        vs_src_yaml = (
            f"{TEST_DATA}"
            f"/virtual-server-route-selector/regexp-location/standard/virtual-server-invalid-duplicate-routes.yaml"
        )
        patch_virtual_server_from_yaml(
            kube_apis.custom_objects, v_s_route_selector_setup.vs_name, vs_src_yaml, v_s_route_selector_setup.namespace
        )
    ...
    def test_flow_for_invalid_vsr(
        ...
        vs_src_yaml = (
            f"{TEST_DATA}" f"/virtual-server-route-selector/regexp-location/standard/virtual-server-exact.yaml"
        )
        patch_virtual_server_from_yaml(
            kube_apis.custom_objects, v_s_route_selector_setup.vs_name, vs_src_yaml, v_s_route_selector_setup.namespace
        )
        vsr_src_yaml = (
            f"{TEST_DATA}"
            f"/virtual-server-route-selector/regexp-location/route-multiple-invalid-multiple-regexp-subroutes.yaml"
        )
        patch_v_s_route_from_yaml(
            kube_apis.custom_objects,
        ...
        ns_events = get_events(kube_apis.v1, v_s_route_selector_setup.route_m.namespace)
        assert_event(vsr_event_text, ns_events) and assert_event(vs_event_text, ns_events)
        assert "location =/backends/exact-match$request {" not in config
...
def vsr_selector_regexp_setup(
    ...
    print("------------------------- Deploy Virtual Server -----------------------------------")
    vs_src_yaml = (
        f"{TEST_DATA}/{request.param['example']}/regexp-location/additional-case/virtual-server-exact-over-all.yaml"
    )
    vs_name = create_virtual_server_from_yaml(kube_apis.custom_objects, vs_src_yaml, test_namespace)
    vs_host = get_first_host_from_yaml(vs_src_yaml)
    ...
    for item in ["prefix", "exact", "regexp"]:
        create_v_s_route_from_yaml(
            kube_apis.custom_objects,
            f"{TEST_DATA}/{request.param['example']}/regexp-location/additional-case/route-{item}.yaml",
            test_namespace,
        )
...
@pytest.mark.vsr_regexes
@pytest.mark.parametrize(
    "crd_ingress_controller, vsr_selector_regexp_setup",
    [
        (
            {"type": "complete", "extra_args": [f"-enable-custom-resources"]},
            {"example": "virtual-server-route-selector"},
...
class TestVSRSelectorRegexpMultipleMatches:
    def test_exact_match_overrides_all(
        self,
        kube_apis,
        ingress_controller_prerequisites,
    ...
    def test_regexp_overrides_prefix(
        ...
        vs_src_yaml = (
            f"{TEST_DATA}"
            f"/virtual-server-route-selector/regexp-location/additional-case/virtual-server-regexp-over-prefix.yaml"
        )
        patch_virtual_server_from_yaml(
            kube_apis.custom_objects,
